...
function MyJobs() {
    ...
    useEffect(() => {
        setIsLoading(true)
        fetch("http://localhost:3000/myJobs/shadrackshibu007@gmail.com")
            .then(res => res.json())
            .then(data =>{ 
                setJobs(data)
            });
    ...
    return (
      ...
      <div className="max-w-screen-2xl h-screen container mx-auto xl:px-24 px-4">
        ...
        <section className="py-1 bg-blueGray-50">
                            ...
                            <div className="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
                                <Link to="/post-job">
                                    <button className="bg-indigo-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150"
                                        type="button">Post A New Job</button>
                                </Link>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <tbody>
                                        {
                                            currentJobs.map((job, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                                        <button className=" bg-gray-500 py-2 px-6 text-white rounded-sm">
                                                            <Link to={`/edit-jobs/${job?._id}`}>Edit</Link>
                                                        </button>
                                                    </td>
                                                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
            ...
            <div className="flex justify-center text-black space-x-8 mb-8">
                {
                currentPage >1 && (
                    < button className="hover:underline" onClick={prevPage}>Previos</button>
                )}
                {
